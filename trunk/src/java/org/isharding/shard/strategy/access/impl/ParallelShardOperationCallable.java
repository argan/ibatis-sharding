 ...
 */
class ParallelShardOperationCallable  implements Callable<Void> {
    ...
    private final ShardOperation           operation;
    ...
    public ParallelShardOperationCallable(CountDownLatch startSignal, CountDownLatch doneSignal,
            ExitStrategy  exitStrategy, ShardOperation  operation, Shard shard,
        ...
        try {
            ...
            if (exitStrategy.addResult(operation.execute(shard), shard)) {
                log.debug(String.format(
                        "Short-circuiting execution of %s on other threads after execution against shard %s", operation
                ...
                log.debug(String.format("Checking %d future tasks to see if they need to be cancelled.", futureTasks
                        .size()));
    ...
    private void waitForStartSignal() {
        try {
            ...
            final String msg = String.format(
                    "Received interrupt while waiting to begin execution of %s against shard %s", operation
                            .getOperationName(), shard);
            log.error(msg);
